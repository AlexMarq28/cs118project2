...
int PacketInRange(int packetSeqNum, int seqNum){
    if(packetSeqNum>seqNum&&packetSeqNum<seqNum+PAYLOAD_SIZE*10){
        return 1;
    }
    if(packetSeqNum<(seqNum+PAYLOAD_SIZE*10)%MAX_SEQN&&(seqNum+PAYLOAD_SIZE*10)>MAX_SEQN){
        return 1;
    }
    return 0;
}
...
int main (int argc, char *argv[])
{
    ...
    int earliestSeqNum=seqNum;
    
    while (1)
    {
	if (total_packets < 10 && !full&&PacketInRange(seqNum, earliestSeqNum))
	{
	    m = fread(buf, 1, PAYLOAD_SIZE, fp);
	    if (m <= 0)
	    {
	...
	{
        if(earliestSeqNum>pkts[i].seqnum)earliestSeqNum=pkts[i].seqnum;
